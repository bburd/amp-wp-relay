...
function log_message($msg) {
    ...
    $logFile = __DIR__ . '/amp-relay.log';
    $timestamp = date('[Y-m-d H:i:s]');
    error_log("$timestamp $msg\n", 3, $logFile);
}

if (LOGGING_ENABLED) {
    file_put_contents(__DIR__ . '/amp-relay.log', "[" . date('c') . "] Script loaded\n", FILE_APPEND);
    log_message("Request started from IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
}

if (!isset($_GET['key']) || $_GET['key'] !== $sharedKey) {
...
function fetch_instance_status($name, $host, $username, $password) {
...
foreach ($instances as $name => $cfg) {
    $result[$name] = fetch_instance_status($name, $cfg['host'], $cfg['username'], $cfg['password']);
}
